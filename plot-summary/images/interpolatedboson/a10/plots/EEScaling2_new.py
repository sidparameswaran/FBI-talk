...
ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: str(IO.round_sig(x, 2))))
plt.draw()
plt.plot(LL(0, 2, 0), E(0, 2, 0), label=labels[0], marker='.',markersize=8, color='b', ls='')
...
7.06/1.67
plt.plot(LL(0, 4, 0), E(0, 4, 0), label=labels[1], marker='.',markersize=8, color='g', ls='')
E(0, 4, 0)[-1]/E(0, 2, 0)[-1]
plt.plot(LL(1, 0, 0), E(1, 0, 0), label=labels[2], marker='.',markersize=8, color='k', ls='')
params, R, line, text = try_fit(LL(1, 0, 0)[1:], E(1, 0, 0)[1:], pl, 'k', [1/2**x for x in LL(1, 0, 0)[1:]], 10.9, [11, 0.8], pl_txt)
plt.plot(LL(1, 2, 0), E(1, 2, 0), label='$j_{-1}\\vert 1, 0 \\rangle$', marker='.',markersize=8, color='c', ls='')
plt.plot(LL(0, 6, 0), E(0, 6, 0), label=labels[3], marker='.',markersize=8, color='r', ls='')
...
labels
plt.xlabel('L', fontsize=8)
plt.ylabel('Entanglement Energy $E=-Log(\\rho/\\rho_0)$', fontsize=8)
plt.title('Entanglement energy versus system size', fontsize=8)
plt.tight_layout()
plt.xlim(xmax=13.1)
text.set_x(9.9)
text.set_y(2.2)
