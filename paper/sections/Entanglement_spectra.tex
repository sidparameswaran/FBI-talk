...
featureless bosonic insulator state \ref{eq:def} could be in a SPT phase. 
However, it's still not clear which subgroup (or subgroups) $G$ of the 
wavefunction's symmetries can be used to protect the SPT phase. We will study 
the action of the symmetries of the wavefunction on the entanglement cut, using the same cylinder geometry as above, to try to determine $G$.

%With a parent Hamiltonian, we could in principal determine $G$ by 
%diagonalizing Hamiltonians with a variety of perturbations that break each 
%combination of symmetries, and seeing which perturbations destroy the 
%entanglement edge. 
...
\cite{pollmann2010}. Each on-site symmetry of the wavefunction $U_g = \otimes_i u^i_g$, with $U_g \ket{\psi} = e^{i \Theta_g} \ket{\psi}$  
